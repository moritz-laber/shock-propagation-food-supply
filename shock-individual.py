...
output_folder = './individual/'     # folder to write results to
...
for cit, c_shock in enumerate(countries):
    ...
    XS = pd.DataFrame(index=ci_index,columns=pd.MultiIndex.from_product([[c_shock],items]))
    ...
    for iit,i_shock in enumerate(items):
        ...
        if producer.loc[(c_shock,i_shock),'is_producer']:

            # Find the shocked country and item in the index
            shock_id = list(ci_index.values).index((c_shock,i_shock))
            ...
            # Save last time step
            XS.loc[idx[:,:],(c_shock,i_shock)] = x.toarray()[:,0]
        
        else:
            XS.loc[idx[:,:],(c_shock,i_shock)] = xbase
